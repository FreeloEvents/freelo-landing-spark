...
const Hero: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      toast.success(language === 'he' ? 'תודה שנרשמת!' : 'Thank you for signing up!', {
        description: language === 'he' ? 'נעדכן אותך לגבי פרילו.' : 'We will keep you updated about Freelo.',
      });
      setEmail('');
      setIsLoading(false);
    }, 1000);
  ...
  return (
    <MotionDiv
      ...
      className={`min-h-screen flex flex-col items-center justify-center px-4 py-16 sm:py-20 relative overflow-hidden ${language === 'he' ? 'text-right' : 'text-left'}`}
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full">
          ...
          <button 
            ...
            disabled={isLoading}
          >
            {isLoading ? (language === 'he' ? 'נרשם...' : 'Signing up...') : t('getEarlyAccess')}
          </button>
        </form>
